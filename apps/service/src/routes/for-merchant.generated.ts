...
import type { MerchantTaxRegistrationsResponse } from "packages/models/src/merchantTaxRegistrationsResponse.generated.ts";
...
app.get(
  ...
  async (c) => {
    ...
    return c.json(
      await getTaxRegistrationsForMerchantId({ db, env: c.env, user: c.var.user, params, query }),
    );
  },
);
...
) => Promise<MerchantTaxRegistrationsResponse>;
